...
unsigned char view_log(unsigned char key, unsigned char reset_flag)
{
    ...
    if(clear_flag == 0)
    {
        if(key == 0x2F && events_logged > 0)
        {
            clear_screen();
            log_pos++;
            log_pos = log_pos%events_logged;
        }
        if(key == 0x37 && events_logged <= 10)
        {
            clear_screen();
            if(log_pos == 0)
            {
                log_pos = events_logged;
            }
            log_pos--;
            log_pos = log_pos%events_logged;
        }
        clcd_print("L TIME     E  SP", (0x80 + 0));
        fetch_logs(log_pos);
        clcd_print(get_log, (0xC0 + 0));
    }
...
void clear_log(unsigned char key)
{
    if(clear_flag == 0)
    {
        ...
        clcd_print("Successfully", (0xC0 + 1));
        log_pos = 0;
    }
}
void download_log()
{
    ...
    {
        clear_screen();
        clcd_print("NO LOGS", (0x80 + 2));
        clcd_print("FOUND", (0xC0 + 2));
...
void save_time(char *ntime)
{
    ...
    write_ds1307(0x02, bcdh);
    write_ds1307(0x01, bcdm);
    write_ds1307(0x00, bcds);
}
